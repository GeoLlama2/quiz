...
export async function POST(request) {
  try {
    // 1. Get lecturer login data
    const reqBody = await request.json();
    const { email, password } = reqBody;
    console.log(reqBody);

    // 2. Check if lecturer exists in DynamoDB
    // Since 'email' is our Partition Key, we use GetCommand to fetch the lecturer directly
    ...
    // If no item is returned, the lecturer doesn't exist
    if (!response.Item) {
      return NextResponse.json(
        { error: "Lecturer with this email not found" },
        { status: 400 }
      );
    }
